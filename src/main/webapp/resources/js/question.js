            ...
            if (data==null||data.length==0){
                ...
                for (var i=1;i<=data.length;i++){
                    if (data[i-1]["info"]=="accepted"){
                        $(".states").append("<div class=\"state-success state well\"><h4>CASE "+i+":Accepted</h4><p>"+data[i-1]["timeCost"]+"ms&nbsp;"+data[i-1]["memoryCost"]+"kb</p></div>");
                    }else{
                        $(".states").append("<div class=\"state-fail state well\"><h4>CASE "+i+"</h4><pre>"+data[i-1]["info"]+"</pre>");
                    }
                }
            }
        }
